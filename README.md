...
```bash
az staticwebapp create -n StarterNextJSTailwindCSS -g StarterNextJS -s https://github.com/<YOUR_GITHUB_USERNAME>/weather-app-example -l WestUs2 -b main --login-with-github
```
...
```bash 
https://github.com/<YOUR_GITHUB_USERNAME>/weather-app-example/actions
```
...
``` bash
sed -i "s/weather-app-example/<your project name>/" package.json package-lock.json README.md
chmod u+x setup.sh
```
